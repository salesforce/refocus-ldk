...
describe('lens init test>', function() {
    ...
    it('lens init', (done) =>{
        ...
        forkedProcess.on('close', () => {
            ...
            expect(fs.readdirSync(`./${projectname}/node_modules`)).to.have.members(['chai',  'dateformat', 'eslint', 'express', 'gulp', 'jsdom', 'mocha', 'rimraf', 'webpack']);

            /* set up package.json & readme */

            expect(fs.existsSync(`./${projectname}/package.json`)).to.be.true;
            const json_file = fs.readJsonSync(`./${projectname}/package.json`);
